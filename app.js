...
document.getElementById('btn-create').addEventListener('click', () => {
    ...
    if (peer) peer.destroy(); // Nettoie si on avait déjà cliqué
    ...
    peer.on('connection', (conn) => {
        ...
        // Fonction sécurisée pour ajouter un client (évite le bug où 'open' ne se lance pas)
        const setupClient = () => {
...
function checkAnswer(peerId, answer) {
    ...
    // Tolérance d'inclusion simple (ex: "jackson" valide "michael jackson")
    if (normTitle.includes(normInput) || normArtist.includes(normInput)) {
        if (normInput.length < 3) return; // Évite de valider avec juste 1 ou 2 lettres
...
document.getElementById('btn-join').addEventListener('click', () => {
    ...
    // Feedback visuel + protection contre le multi-clic
    ...
    peer.on('open', (id) => {
        ...
        // Sécurité : Si l'hôte ne répond pas dans les 5 secondes
        let timeout = setTimeout(() => {
            alert("Code incorrect ou Hôte introuvable !");
            btnJoin.innerText = "Rejoindre une room (Client)";
            btnJoin.disabled = false;
        }, 5000);

        hostConnection.on('open', () => {
            clearTimeout(timeout); // Connexion réussie, on annule le timeout
            btnJoin.innerText = "Rejoindre une room (Client)";
            btnJoin.disabled = false;
            showView('lobby');
        });
    ...
    peer.on('error', (err) => {
        alert('Erreur: Vérifiez le code de la room.');
        btnJoin.innerText = "Rejoindre une room (Client)";
        btnJoin.disabled = false;
    });
});
